...
export function BambooSim() {
  ...
  useEffect(() => {
    ...
    if (!simRef.current) {
      simRef.current = new BambooSimWrapper(ref.current);
    }
  }, []);
				...
				<div className="flex-1 flex flex-col">
						<main className="flex-1 flex flex-col">
								...
								<div className="flex-1 relative">
										<div 
											ref={ref} 
											className={`absolute inset-0 ${activeTab === '3d' ? '' : 'hidden'}`}
											style={{
												position: 'relative',
												width: "100%",
												height: "100%",
										...
										<div className={`absolute inset-0 p-4 overflow-y-auto ${activeTab === 'stats' ? '' : 'hidden'}`}>
